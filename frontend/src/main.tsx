...
type Agent = { name: string; category: string; description: string; skill: string; protocol: string; inputs: string[]; outputs: string[] };

// These cards mirror the four AGENTS_TO_REGISTER definitions and the four implementations in agents/.
const agents: Agent[] = [
  { name: 'Smart Money Rebalancing Agent', category: 'Rebalancing', skill: 'negotiate-erc8183-job', protocol: 'PancakeSwap V3', description: 'Automatically manages PancakeSwap V3 LP ranges on BSC. Detects out-of-range positions and resets to an optimal tick band based on current volatility and fee tier. Returns a signed action report with the new range, estimated gas, and projected fee capture improvement.', inputs: ['wallet_address', 'pool_address', 'current_lower_tick', 'current_upper_tick', 'volatility_multiplier'], outputs: ['new range', 'estimated gas', 'projected fee improvement'] },
  { name: 'Smart Money Grid Trading Agent', category: 'Grid Trading', skill: 'negotiate-erc8183-job', protocol: 'PancakeSwap', description: 'Places and manages automated grid orders on PancakeSwap. Buys low and sells high within a configurable price band, capturing spread from sideways markets 24/7. Returns a trade log with entry/exit prices and P&L.', inputs: ['wallet_address', 'pair', 'capital_usd', 'grid_count', 'range_pct'], outputs: ['grid levels', 'profit estimate', 'risk factors'] },
  { name: 'Smart Money Yield Agent', category: 'Yield Optimisation', skill: 'negotiate-erc8183-job', protocol: 'Aave V3 / Venus / Lista / PancakeSwap', description: 'Routes deposited liquidity to the highest available APR across Aave V3, Venus, Lista Liquid Staking, and PancakeSwap pools on BSC. Returns a ranked APR comparison table with a recommended action.', inputs: ['wallet_address', 'asset_symbol', 'deposit_amount'], outputs: ['ranked APR comparison', 'recommended protocol', 'projected returns'] },
  { name: 'Smart Money Health Factor Agent', category: 'Health Factor Monitoring', skill: 'negotiate-erc8183-job', protocol: 'Aave V3 / Venus', description: 'Monitors Aave V3 and Venus lending positions in real time. Flags positions approaching the liquidation threshold and returns a structured recommendation: top-up collateral amount, repay amount, or safe — with the current health factor and liquidation price.', inputs: ['wallet_address'], outputs: ['health factor', 'collateral/debt summary', 'risk recommendation'] },
];

const API_URL = (import.meta.env.VITE_API_URL || '').replace(/\/$/, '');

function App() {
  const [account, setAccount] = useState(''); const [query, setQuery] = useState(''); const [category, setCategory] = useState('All');
  const [selected, setSelected] = useState<Agent | null>(null); const [quote, setQuote] = useState<any>(null); const [status, setStatus] = useState('');
  const filtered = useMemo(() => agents.filter(a => { const text = `${a.name} ${a.category} ${a.description} ${a.protocol}`.toLowerCase(); return text.includes(query.toLowerCase()) && (category === 'All' || a.category === category); }), [query, category]);

  useEffect(() => { const eth = window.ethereum; if (!eth) return; const onAccounts = (...args: unknown[]) => setAccount(String((args[0] as string[] | undefined)?.[0] || '')); eth.on?.('accountsChanged', onAccounts); return () => eth.removeListener?.('accountsChanged', onAccounts); }, []);

  async function connect() {
    if (!window.ethereum) { setStatus('Install a browser wallet such as MetaMask to connect.'); return; }
    try { const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' }) as string[]; setAccount(accounts[0] || ''); try { await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: '0x61' }] }); } catch { /* Wallet/network selection can be handled by the wallet. */ } setStatus('Wallet connected.'); }
  ...
  async function hire(agent: Agent) {
    setSelected(agent); setQuote(null); setStatus(''); if (!account) { setStatus('Connect your wallet before hiring an agent.'); return; }
    if (!API_URL) { setStatus('Marketplace backend URL is not configured yet. Set VITE_API_URL for the eventual Render API.'); return; }
    try { const response = await fetch(`${API_URL}/a2a`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', id: Date.now(), method: 'message/send', params: { message: { parts: [{ type: 'data', data: { skill: agent.skill, task_description: `[category:${agent.category.toLowerCase().replaceAll(' ', '-').replace('optimisation','yield')}] Hire ${agent.name} for wallet ${account}` } }] } } }) }); const data = await response.json(); setQuote(data?.result?.message?.parts?.[0]?.data || null); if (!response.ok) throw new Error(data?.error || 'Quote request failed'); }
  ...
  return <div className="app">
    ...
    <main>
      ...
      <div className="grid">{filtered.map(agent => <article className="card" key={agent.name}><div className="cardTop"><span className="badge">{agent.category}</span><span className="live">ERC-8004</span></div><div className="agentIcon">{agent.category === 'Rebalancing' ? '↗' : agent.category === 'Grid Trading' ? '▦' : agent.category === 'Yield Optimisation' ? '◈' : '♥'}</div><h3>{agent.name}</h3><p>{agent.description}</p><div className="meta"><span>PROTOCOL<b>{agent.protocol}</b></span><span>JOB SKILL<b>{agent.skill}</b></span></div><div className="cardActions"><button className="details" onClick={() => {setSelected(agent);setQuote(null)}}>View capabilities</button><button className="hire" onClick={() => hire(agent)}>Hire agent →</button></div></article>)}</div></section>
