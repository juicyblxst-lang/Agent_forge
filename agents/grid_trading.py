...
def _fetch_price(pair: str = "WBNB/USDT") -> float:
    ...
    try:
        ...
        price = float(r.json().get("data", {}).get("price", 0))
        return price if price > 0 else 300.0
    except Exception:
        return 300.0
...
ACTION STEPS:
  ...
  3. Monitor for range breakout -- reset grid if price moves >25% out""".strip()
